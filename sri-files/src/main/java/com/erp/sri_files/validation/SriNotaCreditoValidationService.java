...
@Service
public class SriNotaCreditoValidationService {

    public NotaCreditoValidationResult validate(String xml) {
        List<String> errors = new ArrayList<>();
        List<String> warnings = new ArrayList<>();

        if (xml == null || xml.isBlank()) {
            errors.add("XML vacío o nulo");
            return new NotaCreditoValidationResult(false, errors, warnings, null, null, null, null);
        }
        ...
        try {
            ...
            if (!"notaCredito".equals(rootName)) {
                errors.add("Raíz inválida. Se esperaba <notaCredito> y se recibió <" + rootName + ">");
            }
            ...
            if (version == null || version.isBlank()) {
                errors.add("El atributo version es obligatorio en <notaCredito>");
            } else if (!version.matches("1\\.0\\.0")) {
                warnings.add("Versión de nota de crédito no reconocida: " + version);
            }
            ...
            if (!"04".equals(codDoc)) {
                errors.add("codDoc inválido para nota de crédito. Se esperaba 04 y se recibió " + empty(codDoc));
            }
            ...
            require(errors, text(doc, "fechaEmision"), "infoNotaCredito.fechaEmision");
            require(errors, text(doc, "tipoIdentificacionComprador"), "infoNotaCredito.tipoIdentificacionComprador");
            require(errors, text(doc, "identificacionComprador"), "infoNotaCredito.identificacionComprador");
            require(errors, text(doc, "razonSocialComprador"), "infoNotaCredito.razonSocialComprador");

            String codDocModificado = text(doc, "codDocModificado");
            require(errors, codDocModificado, "infoNotaCredito.codDocModificado");
            require(errors, text(doc, "numDocModificado"), "infoNotaCredito.numDocModificado");
            require(errors, text(doc, "fechaEmisionDocSustento"), "infoNotaCredito.fechaEmisionDocSustento");
            require(errors, text(doc, "motivo"), "infoNotaCredito.motivo");

            String valorModificacion = text(doc, "valorModificacion");
            require(errors, valorModificacion, "infoNotaCredito.valorModificacion");

            validateClaveAcceso(errors, warnings, claveAcceso, codDoc, ruc, ambiente);

            return new NotaCreditoValidationResult(
    ...
    public record NotaCreditoValidationResult(
        boolean valid,
        List<String> errors,

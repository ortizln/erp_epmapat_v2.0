...
@Service
public class SriNotaDebitoValidationService {

    public NotaDebitoValidationResult validate(String xml) {
        List<String> errors = new ArrayList<>();
        List<String> warnings = new ArrayList<>();

        if (xml == null || xml.isBlank()) {
            errors.add("XML vacío o nulo");
            return new NotaDebitoValidationResult(false, errors, warnings, null, null, null, null);
        }
        ...
        try {
            ...
            if (!"notaDebito".equals(rootName)) {
                errors.add("Raíz inválida. Se esperaba <notaDebito> y se recibió <" + rootName + ">");
            }
            ...
            if (version == null || version.isBlank()) {
                errors.add("El atributo version es obligatorio en <notaDebito>");
            } else if (!version.matches("1\\.0\\.0")) {
                warnings.add("Versión de nota de débito no reconocida: " + version);
            }
            ...
            if (!"05".equals(codDoc)) {
                errors.add("codDoc inválido para nota de débito. Se esperaba 05 y se recibió " + empty(codDoc));
            }
            ...
            require(errors, text(doc, "fechaEmision"), "infoNotaDebito.fechaEmision");
            require(errors, text(doc, "tipoIdentificacionComprador"), "infoNotaDebito.tipoIdentificacionComprador");
            require(errors, text(doc, "identificacionComprador"), "infoNotaDebito.identificacionComprador");
            require(errors, text(doc, "razonSocialComprador"), "infoNotaDebito.razonSocialComprador");

            require(errors, text(doc, "codDocModificado"), "infoNotaDebito.codDocModificado");
            require(errors, text(doc, "numDocModificado"), "infoNotaDebito.numDocModificado");
            require(errors, text(doc, "fechaEmisionDocSustento"), "infoNotaDebito.fechaEmisionDocSustento");

            String totalSinImpuestos = text(doc, "totalSinImpuestos");
            require(errors, totalSinImpuestos, "infoNotaDebito.totalSinImpuestos");
            String valorTotal = text(doc, "valorTotal");
            require(errors, valorTotal, "infoNotaDebito.valorTotal");

            NodeList motivoNodes = doc.getElementsByTagName("motivo");
            if (motivoNodes.getLength() == 0) {
                errors.add("Debe tener al menos un motivo en infoNotaDebito.motivos");
            }

            validateClaveAcceso(errors, warnings, claveAcceso, codDoc, ruc, ambiente);

            return new NotaDebitoValidationResult(
    ...
    public record NotaDebitoValidationResult(
        boolean valid,
        List<String> errors,

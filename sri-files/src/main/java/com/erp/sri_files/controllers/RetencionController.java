...
@Tag(name = "Retenciones", description = "Gestión de retenciones electrónicas")
public class RetencionController {

    private static final Logger log = LoggerFactory.getLogger(RetencionController.class);

    private final SriRetencionValidationService validationService;
    private final SignService signService;
    private final SriGateway sriGateway;
    private final DocumentoProcessManager processManager;

    @Operation(summary = "Crear y procesar retención electrónica", description = "Recibe un XML de retención, lo valida, firma y envía al SRI para autorización",
        responses = {
            @ApiResponse(responseCode = "200", description = "Retención autorizada por SRI"),
            @ApiResponse(responseCode = "202", description = "Retención pendiente de autorización"),
            @ApiResponse(responseCode = "400", description = "Validación previa fallida o SRI rechazó"),
            @ApiResponse(responseCode = "500", description = "Error interno del servidor")
        })
    @PostMapping(consumes = MediaType.APPLICATION_XML_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> crear(
            @Parameter(description = "XML de la retención electrónica", required = true)
        ...
        try {
            ...
            log.info("Recibida retención para procesar [requestId={}]", requestId);

            var validation = validationService.validate(xmlPlano);
            if (!validation.valid()) {
                log.warn("Retención rechazada por validación previa: {} [requestId={}]", 
            ...
            if (!firmaResult.exitoso()) {
                log.error("Error firmando retención: {} [requestId={}]", firmaResult.mensaje(), requestId);
                return ResponseEntity.badRequest().body(Map.of(
            ...
            if (recepcionResult.resultado() != SriGateway.ResultadoRecepcion.RECIBIDA) {
                log.warn("Retención no recibida por SRI: {} [requestId={}]", 
            ...
            if (authResult.autorizado()) {
                log.info("Retención autorizada: nro={} [requestId={}]", 
            ...
            log.error("Error procesando retención [requestId={}]", requestId, e);
            return ResponseEntity.status(500).body(Map.of(
                "error", "Error interno procesando retención",
    ...
    @Operation(summary = "Validar XML de retención", description = "Valida un XML de retención contra las reglas del SRI antes de enviarlo",
        responses = {
    ...
    public ResponseEntity<?> validar(
            @Parameter(description = "XML de la retención a validar", required = true)
            @RequestBody String xmlBody) {
        String requestId = UUID.randomUUID().toString();
        MDC.put("requestId", requestId);
        ...
        try {
            ...
            log.error("Error validando retención [requestId={}]", requestId, e);
            return ResponseEntity.status(500).body(Map.of(
                "error", "Error interno validando retención",
                "detalle", e.getMessage(),
                "requestId", requestId
            ));
        } finally {
    ...
    @Operation(summary = "Consultar autorización SRI", description = "Consulta el estado de autorización de una retención en el SRI",
        responses = {
    ...
    public ResponseEntity<?> consultarAutorizacion(
        ...
        try {
            ...
            log.error("Error consultando autorización retención [requestId={}]", requestId, e);
            return ResponseEntity.status(500).body(Map.of(

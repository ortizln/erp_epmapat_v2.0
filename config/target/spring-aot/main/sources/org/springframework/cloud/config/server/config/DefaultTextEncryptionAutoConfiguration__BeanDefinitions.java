...
public class DefaultTextEncryptionAutoConfiguration__BeanDefinitions {
  ...
  private static BeanInstanceSupplier<KeyProperties> getKeyPropertiesInstanceSupplier() {
    return BeanInstanceSupplier.<KeyProperties>forFactoryMethod(DefaultTextEncryptionAutoConfiguration.class, "keyProperties")
            .withGenerator((registeredBean) -> registeredBean.getBeanFactory().getBean("org.springframework.cloud.config.server.config.DefaultTextEncryptionAutoConfiguration", DefaultTextEncryptionAutoConfiguration.class).keyProperties());
  }

...
public interface TramiteNuevoR extends JpaRepository<TramiteNuevo, Long> {
	...
	// @Query(value = "DELETE FROM tramitenuevo AS tn WHERE NOT EXISTS(SELECT * FROM
	// clientes AS c WHERE c.idnacionalidad_nacionalidad=n.idnacionalidad)AND
	// n.idnacionalidad=?1 ", nativeQuery = true)
	// void deleteByIdQ(Long id);
	// @Query(value = "SELECT * FROM nacionalidad AS n WHERE EXISTS(SELECT * FROM
	// clientes AS c WHERE c.idnacionalidad_nacionalidad=n.idnacionalidad)AND
	// n.idnacionalidad=?1 ", nativeQuery = true)

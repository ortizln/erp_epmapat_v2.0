...
import com.erp.contabilidad.servicio.IfinanServicio;
...
public class IfinanApi {
    ...
    public List<Ifinan> getIfinan(@Param(value = "codifinan") String codifinan,
        ...
        if (codifinan != null) return ifinanServicio.findByCodifinan(codifinan);
        if (nomifinan != null) return ifinanServicio.findByNomifinan(nomifinan.toLowerCase());
        return null;
    }

    @PostMapping
    public Ifinan updateOrSave(@RequestBody Ifinan x) { return ifinanServicio.save(x); }

    @GetMapping("/{idifinan}")
    public ResponseEntity<Ifinan> getByIdNovedad(@PathVariable Long idifinan) {
        Ifinan x = ifinanServicio.findById(idifinan)
                .orElseThrow(() -> new ResourceNotFoundExcepciones(("No existe Institución con Id: " + idifinan)));
    ...
    public ResponseEntity<Ifinan> update(@PathVariable Long idifinan, @RequestBody Ifinan x) {
        Ifinan y = ifinanServicio.findById(idifinan)
                .orElseThrow(() -> new ResourceNotFoundExcepciones(("No existe Institución con Id: " + idifinan)));
        return ResponseEntity.ok(ifinanServicio.save(y));
    }
}
